...
require_once DOCROOT.'modules/DynamicFields/DynamicField.php';
...
class SugarBean
{
     ...
     * @see is_AuditEnabled
     */
    final public function getAuditEnabledFieldDefinitions()
    {
        ...
        if (! isset($this->audit_enabled_fields)) {
            ...
            foreach ($this->field_defs as $field => $properties) {
                if ((! empty($properties['Audited']) || ! empty($properties['audited']))) {
     ...
     * @return boolean
     */
    final public function is_AuditEnabled()
    {
        global $dictionary;

        return array_get($dictionary, "{$this->getObjectName()}.audited", false);
    }
     ...
     */
    final public function get_audit_table_name()
    {
     ...
     */
    final public function get_custom_table_name()
    {
     ...
     */
    final public function create_audit_table()
    {
        global $dictionary;
    ...
    final public function getTableName()
    {
        ...
        return array_get($dictionary, "{$this->getObjectName()}.table");
    }
     ...
     */
    final public function getIndices()
    {
        global $dictionary;

        return array_get($dictionary, "{$this->getObjectName()}.indices", []);
    }
     ...
     */
    final public function getFieldDefinition($name)
    {
        return array_get($this->field_defs, $name, false);
    }

    /**
     * Return definition for the id field name.
     *
     * The definitions were loaded in the constructor.
     *
     * @return array Field properties.
     */
    final public function getPrimaryFieldDefinition()
    {
    ...
    public function unPopulateDefaultValues()
    {
        ...
        foreach ($this->field_defs as $field => $value) {
            ...
            if (! empty($this->$field) && ! empty($value['display_default']) && in_array($value['type'], [
                    'date',
                    'datetime',
                    'datetimecombo'
     ...
     */
    final public static function removeRelationshipMeta($key, $db, $tablename, $dictionary, $module_dir)
    {
        ...
        if (! is_array($dictionary) or ! array_key_exists($key, $dictionary)) {
            Log::fatal("removeRelationshipMeta: Metadata for table ".$tablename." does not exist");
     ...
     *
     * It is called during setup/install. It is used statically to create relationship meta data for many-to-many
     * tables.
     *
     ...
     * @param         string     module_dir name of subdirectory where module is installed.
     * @param boolean $iscustom  Optional,set to true if module is installed in a custom directory. Default value is
     *                           false.
     *
     *  Internal function, do not override.
     */
    final public static function createRelationshipMeta($key, $db, $tablename, $dictionary, $module_dir, $iscustom = false)
    {
        //load the module dictionary if not supplied.
        if (empty($dictionary) && ! empty($module_dir)) {
            ...
            if (file_exists($filename)) {
                ...
                Log::debug("createRelationshipMeta: no metadata file found".$filename);
        ...
        if (! is_array($dictionary) or ! array_key_exists($key, $dictionary)) {
            Log::fatal("createRelationshipMeta: Metadata for table ".$tablename." does not exist");
            display_notice("meta data absent for table ".$tablename." keyed to $key ");
        } else {
            if (isset($dictionary[$key]['relationships'])) {
                ...
                foreach ($RelationshipDefs as $rel_name => $rel_def) {
                    if (isset($rel_def['lhs_module']) and ! isset($beanList_ucase[strtoupper($rel_def['lhs_module'])])) {
                        Log::debug('skipping orphaned relationship record '.$rel_name.' lhs module is missing '.$rel_def['lhs_module']);
                        continue;
                    }
                    if (isset($rel_def['rhs_module']) and ! isset($beanList_ucase[strtoupper($rel_def['rhs_module'])])) {
                        Log::debug('skipping orphaned relationship record '.$rel_name.' rhs module is missing '.$rel_def['rhs_module']);
                        continue;
                    }

                    //check whether relationship exists or not first.
                    if (Relationship::exists($rel_name, $db)) {
                        Log::debug('Skipping, reltionship already exists '.$rel_name);
                    } else {
    ...
    /**
     * Loads the request relationship. This method should be called before performing any operations on the related
     * data.
     *
    ...
    {
        Log::debug("SugarBean[{$this->object_name}].load_relationships, Loading relationship (".$rel_name.").");

        if (empty($rel_name)) {
            Log::error("SugarBean.load_relationships, Null relationship name passed.");
            return false;
        }
        ...
        Log::debug("SugarBean.load_relationships, Error Loading relationship (".$rel_name.")");
     ...
     */
    final public function load_relationships()
    {
        Log::debug("SugarBean.load_relationships, Loading all relationships of type link.");
     ...
     * @param string $bean_name      name  class name of the related bean.
     ...
     * @return array
     */
    final public function get_linked_beans($field_name, $bean_name, $sort_array = [], $begin_index = 0, $end_index = -1, $deleted = 0, $optional_where = "")
    {
     ...
     * @return array List of fields.
     */
    final public function get_importable_fields()
    {
        $importableFields = [];
        ...
        if (! empty($fieldDefs)) {
            foreach ($fieldDefs as $key => $value_array) {
                if ((isset($value_array['importable']) && (is_string($value_array['importable']) && $value_array['importable'] == 'false' || is_bool($value_array['importable']) && $value_array['importable'] == false)) || (isset($value_array['type']) && $value_array['type'] == 'link') || (isset($value_array['auto_increment']) && ($value_array['type'] == true || $value_array['type'] == 'true'))) {
                    // only allow import if we force it
                    if (isset($value_array['importable']) && (is_string($value_array['importable']) && $value_array['importable'] == 'true' || is_bool($value_array['importable']) && $value_array['importable'] == true)) {
     ...
     * @return array List of fields.
     */
    final public function get_related_fields()
    {
        $related_fields = [];
        $fieldDefs = $this->getFieldDefinitions();
    ...
    public function delete_linked($id)
    {
        ...
        foreach ($linked_fields as $name => $value) {
            if ($this->load_relationship($name)) {
                ...
                Log::fatal("error loading relationship $name");
            }
        }
    }
    ...
    public function create_tables()
    {
        ...
        if (! array_key_exists($key, $dictionary)) {
            Log::fatal("create_tables: Metadata for table ".$this->table_name." does not exist");
    ...
    public function drop_tables()
    {
        ...
        if (! array_key_exists($key, $dictionary)) {
            Log::fatal("drop_tables: Metadata for table ".$this->table_name." does not exist");
     ...
     * @return bool
     */
    protected function has_been_modified_since($date, $modified_user_id)
    {
        global $current_user;
    ...
    protected function send_assignment_notifications($notify_user, $admin)
    {
        ...
        if (($this->object_name == 'Meeting' || $this->object_name == 'Call') || $notify_user->receive_notifications) {
            ...
            if (empty($sendToEmail)) {
                Log::warn("Notifications: no e-mail address set for user {$notify_user->user_name}, cancelling send");
                $sendEmail = false;
            }
            ...
            if ($sendEmail) {
                ...
                if (! $smtpVerified) {
                    Log::fatal("Notifications: error sending e-mail, smtp server was not found ");
                ...
                if (! $notify_mail->Send()) {
                    Log::fatal("Notifications: error sending e-mail (method: {$notify_mail->Mailer}), (error: {$notify_mail->ErrorInfo})");
                } else {
    ...
    {
        global $sugar_version, $sugar_config, $app_list_strings, $current_user, $locale, $beanList;

        $OBCharset = $locale->getPrecedentPreference('default_email_charset');

        $notify_address = $notify_user->emailAddress->getPrimaryAddress($notify_user);
        $notify_name    = $notify_user->full_name;
        Log::debug("Notifications: user has e-mail defined");
     ...
     *
     * @param string $new_rel_id   String of the ID to add
        ...
        if (isset($this->relationship_fields) && is_array($this->relationship_fields)) {
            foreach ($this->relationship_fields as $id => $rel_name) {
                ...
                if (! empty($this->$id)) {
                    ...
                    Log::debug('save_relationship_changes(): From relationship_field array - adding a relationship record: '.$rel_name.' = '.$this->$id);
                    ...
                    if (! empty($this->rel_fields_before_value[$id])) {
                        Log::debug('save_relationship_changes(): From relationship_field array - attempting to remove the relationship record, using relationship attribute'.$rel_name);
                        $this->load_relationship($rel_name);
    ...
    protected function update_parent_relationships($exclude = [])
    {
        foreach ($this->field_defs as $def) {
            if (! empty($def['type']) && $def['type'] == "parent") {
                ...
                if (//First check if the fetched row parent existed and now we no longer have one
                    (! empty($this->fetched_row[$typeField]) && ! empty($this->fetched_row[$idField]) && (empty($this->$typeField) || empty($this->$idField))) || //Next check if we have one now that doesn't match the fetch row
                    (! empty($this->$typeField) && ! empty($this->$idField) && (empty($this->fetched_row[$typeField]) || empty($this->fetched_row[$idField]) || $this->fetched_row[$idField] != $this->$idField)) || // Check if we are deleting the bean, should remove the bean from any relationships
                    $this->deleted == 1
                ) {
                    $parentLinks = [];
    ...
    /**
     * Finally, we update a field listed in the _REQUEST['%/relate_id']/_REQUEST['relate_to'] mechanism (if it has not
     ...
     *
     * This function will be removed in a future release, it is only here to assist upgrading existing code that
     * expects formatted data in the bean
     */
    public function fixUpFormatting()
    {
        global $timedate;
        static $boolean_false_values = ['off', 'false', '0', 'no'];
        ...
        foreach ($this->field_defs as $field => $def) {
            ...
            if ($reformatted) {
                Log::deprecated('Formatting correction: '.$this->module_dir.'->'.$field.' had formatting automatically corrected. This will be removed in the future, please upgrade your external code');
            }
        }
    }
     ...
     *
     * @param int|string $id      Optional, default -1, is set to -1 id value from the bean is used, else, passed value is
     *                            used
     ...
     */
    final public function populateFromRow($row)
    {
        $nullvalue = '';
     ...
     * @param bool|int   $show_deleted Optional, default 0, if set to 1 system will show deleted records.
     * @param bool       $singleSelect
     * @param array      $select_fields
     *
     * @return array Fetched data.
     */
    final public function get_list($order_by = "", $where = "", $row_offset = 0, $limit = -1, $max = -1, $show_deleted = 0, $singleSelect = false, $select_fields = [])
    {
     ...
     */
    final public function process_order_by($order_by, $submodule = null, $suppress_table_name = false)
    {
        if (empty($order_by)) {
            return $order_by;
        }
                ...
                if (isset($list_column[1])) {
                    switch (strtolower($list_column[1])) {
                        ...
                        case 'desc':
                            break;
                        default:
                            Log::debug("process_order_by: ($list_column[1]) is not a valid order.");
                            unset($list_column[1]);
                            break;
                    }
                }
            } else {
                Log::debug("process_order_by: ($list_column[0]) does not have a vardef entry.");
            }
     ...
     *
     * It is intended for use in navigation buttons on the DetailView.  It will pass an offset and limit argument to
     * the sql query.
     *
     * @param string   $order_by
     * @param string   $where        Additional where clause
     * @param int      $offset
     * @param int      $row_offset   Optaional,default 0, starting row number
     * @param init|int $limit        Optional, default -1
     * @param int      $max          Optional, default -1
     * @param bool|int $show_deleted Optioanl, default 0, if set to 1 system will show deleted records.
     *
     * @return array Fetched data.
     */
    final public function get_detail($order_by = "", $where = "", $offset = 0, $row_offset = 0, $limit = -1, $max = -1, $show_deleted = 0)
    {
        Log::debug("get_detail:  order_by = '$order_by' and where = '$where' and limit = '$limit' and offset = '$offset'");
        if (isset($_SESSION['show_deleted'])) {
            $show_deleted = 1;
        }
     ...
     * @return array Fetched data.
     */
    final public function get_related_list($child_seed, $related_field_name, $order_by = "", $where = "", $row_offset = 0, $limit = -1, $max = -1, $show_deleted = 0)
    {
        ...
        Log::debug("get_related_list:  order_by = '$order_by' and where = '$where' and limit = '$limit'");
        if (isset($_SESSION['show_deleted'])) {
            $show_deleted = 1;
        }
        ...
        if ($this->$related_field_name instanceof Link) {
            ...
            $query_array = $this->$related_field_name->getQuery([
     ...
     * @return array
     */
    final public function get_union_related_list(
        $parentbean, $order_by = "", $sort_order = '', $where = "", $row_offset = 0, $limit = -1, $max = -1,
        $show_deleted = 0, $subpanel_def
    ) {
        $secondary_queries = [];
        global $layout_edit_mode, $beanFiles, $beanList;
    ...
    {
        Log::debug("get_full_list:  order_by = '$order_by' and where = '$where'");
        if (isset($_SESSION['show_deleted'])) {
            $show_deleted = 1;
        }
    ...
    /**
     * Return the list query used by the list views and export button. Next generation of create_new_list_query
     * function.
     *
    ...
    public function create_new_list_query(
        $order_by, $where, $filter = [], $params = [], $show_deleted = 0, $join_type = '', $return_array = false,
            ...
            if (empty($this->field_defs[$field]) || ! empty($value['force_blank'])) {
                if (! empty($filter) && isset($filter[$field]['force_exists']) && $filter[$field]['force_exists']) {
                    if (isset($filter[$field]['force_default'])) {
                        ...
                        // spaces are a fix for length issue problem with unions.  The union only returns the maximum number
                        // of characters from the first select statement.
                        ...
                        if ($join['type'] == 'many-to-many') {
                            if (empty($ret_array['secondary_select'])) {
                                ...
                                if (! empty($beanFiles[$beanList[$rel_module]]) && $join_primary) {
                                    ...
                                    } else if (isset($rel_mod->field_defs['created_by'])) {
                            ...
                            } else if (! isset($data['relationship_fields'])) {
     ...
     * @return array
     */
    final public function retrieve_parent_fields($type_info)
    {
        $queries = [];
                ...
                if ($child_info['type'] == 'parent') {
                    if (empty($templates[$child_info['parent_type']])) {
                        ...
                        if (empty($beanFiles[$class])) {
                            Log::error($this->object_name.'::retrieve_parent_fields() - cannot load class "'.$class.'", skip loading.');
                            continue;
                        }
     ...
     * @return array Fetched data
     */
    final public function process_list_query($query, $row_offset, $limit = -1, $max_per_page = -1, $where = '')
    {
        global $sugar_config;
        ...
        while ($max_per_page == -99 || ($index < $row_offset + $max_per_page)) {
            ...
            foreach ($this->field_defs as $field => $value) {
                if (isset($row[$field])) {
                    ...
                    Log::debug("$temp->object_name({$row['id']}): ".$field." = ".$temp->$field);
     ...
     */
    final public function _get_num_rows_in_query($query, $is_count_query = false)
    {
        $num_rows_in_query = 0;
        if (! $is_count_query) {
     ...
     * @return array Fetched data.
     */
    final public function process_union_list_query(
        $parent_bean, $query, $row_offset, $limit = -1, $max_per_page = -1, $where = '', $subpanel_def,
        $query_row_count = '', $secondary_queries = []
    ) {
        $db = DBManagerFactory::getInstance('listviews');
        ...
        Log::debug("process_union_list_query: ".$query);
        if ($max_per_page == -1) {
     ...
     * @return array Fetched data.
     */
    final public function process_detail_query($query, $row_offset, $limit = -1, $max_per_page = -1, $where = '', $offset = 0)
    {
        global $sugar_config;
        Log::debug("process_detail_query: ".$query);
     ...
     * @return array Fetched data.
     */
    final protected function process_full_list_query($query, $check_date = false)
    {
        Log::debug("process_full_list_query: query is ".$query);
        $result = $this->db->query($query, false);
        Log::debug("process_full_list_query: result is ".print_r($result, true));
        $class       = get_class($this);
        $isFirstTime = true;
        $bean        = new $class();
        ...
        while (($row = $bean->db->fetchByAssoc($result)) != null) {
            ...
            foreach ($bean->field_defs as $field => $value) {
                if (isset($row[$field])) {
                    ...
                    Log::debug("process_full_list: $bean->object_name({$row['id']}): ".$field." = ".$bean->$field);
                } else {
                    $bean->$field = '';
                }
            }
     ...
     */
    final protected function track_view($user_id, $current_module, $current_view = '')
    {
    ...
    function fill_in_additional_parent_fields()
    {
        ...
        if (! empty($this->parent_type)) {
            ...
            $this->getRelatedFields($this->parent_type, $this->parent_id, [
                'name'          => 'parent_name',
     ...
     * @param array $def
     */
    protected function fill_in_link_field($linkFieldName, array $def)
    {
     ...
     */
    protected function fill_in_relationship_fields()
    {
     ...
     * @param string $id
     */
    final protected function mark_undeleted($id)
    {
        // call the custom business logic
        $custom_logic_arguments['id'] = $id;
        ...
        foreach ($files as $file) {
            if (file_exists('upload://'.$file)) {
                if (! sugar_rename('upload://'.$file, 'upload://deleted/'.$directory.'/'.$file)) {
                    Log::error('Could not move file '.$file.' to deleted directory');
                }
            }
        }
        ...
        foreach ($files as $file) {
            if (sugar_is_file('upload://deleted/'.$directory.'/'.$file)) {
                if (! sugar_rename('upload://deleted/'.$directory.'/'.$file, 'upload://'.$file)) {
                    Log::error('Could not move file '.$directory.'/'.$file.' from deleted directory');
                }
            }
        }
     ...
     * @return array
     */
    final protected function build_related_list2($query, &$template, &$field_list)
    {
        Log::debug("Finding linked values $this->object_name: ".$query);
    ...
    public function getRelatedFields($module, $id, $fields, $return_array = false)
    {
        ...
        foreach ($fields as $field => $alias) {
            ...
            } else if (! empty($GLOBALS['dictionary'][$object]['fields'][$field]) && (empty($GLOBALS['dictionary'][$object]['fields'][$field]['source']) || $GLOBALS['dictionary'][$object]['fields'][$field]['source'] != "non-db")) {
     ...
     * @return XTemplate
     */
    public function &parse_additional_headers( XTemplate & $list_form, $xTemplateSection)
    {
        return $list_form;
    }
    ...
    /**
     * @param string $table
     * @param array  $relate_values
     * @param bool   $check_duplicates
     * @param bool   $do_update
     * @param null   $data_values
     */
    function set_relationship($table, array $relate_values, $check_duplicates = true, $do_update = false, $data_values = null)
    {
        $where = '';
     ...
     * @return array
     */
    public function retrieve_relationships($table, array $values, $select_id)
    {
    ...
    public function getRealKeyFromCustomFieldAssignedKey($name)
    {
        if ($this->custom_fields->avail_fields[$name]['ext1']) {
            ...
            Log::fatal("SUGARBEAN: cannot find Real Key for custom field of type dropdown - cannot return Value.");
     ...
     *
     * @param string      $view     string required, the view to determine access for i.e. DetailView, ListView...
     * @param bool|string $is_owner bool optional, this is part of the ACL check if the current user is an owner they
     *                              will receive different access
     *
     * @return bool
     */
    ...
    public function getOwnerWhere($user_id)
    {
        ...
        if (!is_null($field)) {
            return " {$this->getTableName()}.{$field} ='{$user_id}' ";
        }
     ...
     * @return string
     */
    public function convertField($fieldvalue, array $fieldDef)
    {
        if (! empty($fieldvalue)) {
            if (! (isset($fieldDef['source']) && ! in_array($fieldDef['source'], [
                    'db',
                    'custom_fields',
                    'relate'
     ...
     */
    final public function loadFromRow($arr)
    {
    ...
    {
        return ! empty($GLOBALS['dictionary'][$this->getObjectName()]['custom_fields']);
    }
     ...
     * @return  string
     */
    final public function create_qualified_order_by($order_by, $qualify)
    {
        // if the column is empty, but the sort order is defined, the value will throw an error,
        // so do not proceed if no order by is given
        if (empty($order_by)) {
            return $order_by;
        }
     ...
     */
    function add_address_streets($street_field)
    {
    ...
    private function _sendNotifications($check_notify)
    {
        ...
        )  // cn: bug 42727 no need to send email to owner (within workflow)
        {
            ...
            if ($admin->settings['notify_on']) {
                Log::info("Notifications: user assignment has changed, checking if user receives notifications");
                $sendNotifications = true;
            } elseif (isset($_REQUEST['send_invites']) && $_REQUEST['send_invites'] == 1) {
                // cn: bug 5795 Send Invites failing for Contacts
                $sendNotifications = true;
            } else {
                Log::info("Notifications: not sending e-mail, notify_on is set to OFF");
            }
     ...
     * @param $key        string the type of field array we are referencing, i.e. list_fields, column_fields,
     *                    required_fields
     *                    *
     *
     * @return null|string
    ...
    {
        if (! empty($this->field_defs['email_addresses']) && $this->field_defs['email_addresses']['type'] == 'link' && ! empty($this->field_defs['email_addresses_non_primary']) && $this->field_defs['email_addresses_non_primary']['type'] == 'email') {
            return true;
        } else {
            return false;
        }
    ...
    {
        return isset($this->description) ? (string)$this->description : "";
    }
     ...
     */
    public function getRelatedObject($field)
    {
        $fieldName = strtolower($field);
        $field     = array_get($this->getFieldDefinitions(), $fieldName, []);
        $type      = array_get($field, 'type');

        if (! in_array($type, ['relate', 'link'])) {
            return null;
        }

        if ($type == 'link') {
            if ($this->load_relationship($fieldName)) {
                return $this->{$fieldName}->getBeans();
            }
        ...
        return call_user_func($beanList[$module].'::load', $this->{$field['id_name']});
    }
    ...
    {
        if (strpos($method, 'getRelated') === 0) {
            return $this->getRelatedObject(substr($method, 10), $parameters);
        }
