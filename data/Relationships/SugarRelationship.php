...
abstract class SugarRelationship
{
    ...
    /**
     * A relationship is self referencing when LHS module = RHS Module
     *
     * @var bool
     */
    protected $self_referencing = false;
     ...
     * See Link2->getQuery
     *
     * @param Link2 $link Link Object to get query for.
     * @param array $params
     *
     ...
     * @param string    $link_name name of link being triggeed
     *
    ...
    /**
     * @param array $optional_array clause to add to the where query when populating this relationship.
     *                              It should be in the
     *
     * @return string
     */
    ...
    public function isParentRelationship()
    {
        ...
        if (! empty($this->def["relationship_role_column"]) && ! empty($this->def["relationship_role_column_value"]) && $this->def["relationship_role_column"] == "parent_type" && $this->def['rhs_key'] == "parent_id") {
            return true;
        }
