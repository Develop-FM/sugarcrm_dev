...
{
    /**
     * relationship object this link is tied to.
     *
    ...
    /**
     * SugarBean this link uses as the context for its calls.
     *
     * @var SugarBean
     */
    protected $focus;
    ...
    /**
     * beans on the other side of the link
     *
     * @var SugarBean[]
     */
    protected $beans;
    ...
    /**
     * Used to store unsaved beans on this relationship that will be combined with the ones
     * pulled from the DB if getBeans() is called.
     *
     * @var SugarBean[]
     */
    protected $tempBeans;
    ...
    /**
     * any additional fields on the relationship
     *
     * @var array
     */
    protected $rows;
    ...
    /**
     * true if this link has been loaded from the database
     *
     * @var bool
     */
    protected $loaded;
    ...
    /**
     * @param string     $linkName String name of a link field in the module's vardefs
     * @param SugarBean  $bean     SugarBean focus bean for this link (one half of a relationship)
     * @param array|bool $linkDef  Array Optional vardef for the link in case it can't be found in the passed in bean
     *                             for the global dictionary
     */
    public function __construct($linkName, SugarBean $bean, $linkDef = false)
    {
        $this->focus = $bean;
    ...
    {
        return BeanFactory::getBean($this->getRelatedModuleName())->getTableName();
    }
     ...
     * @param string $name
     *
     * @return mixed The value for the relationship field $name
     */
    ...
    public function getSide()
    {
        ...
        if (! empty($this->def['side'])) {
            if ((strtolower($this->def['side']) == 'left' || $this->def['side'] == REL_LHS) //Some relationships make have left in the vardef erroneously if generated by module builder
        ...
        } else if (! empty($this->def['id_name'])) { //Next try using the id_name and relationship join keys
     ...
     *                      record will be returned.</li></ul>
     *
     * @return SugarBean[] of SugarBeans related through this link.
     */
     ...
     *
     * @param null|string $id id of related record to retrieve
     *
     * @return SugarBean
     */
    protected function getRelatedBean($id = null)
    {
    ...
    {
        if (! $this->beansAreLoaded() && isset($this->tempBeans[$bean->id])) {
     ...
     * @deprecated
     *
     * @param string $table_name      string relationship table
     * @param array  $join_key_values array of key=>values to identify this relationship by
     *
     ...
     *
     * @return array of keys for duplicate checking, first check for an index of type alternate_key, if not found
     *               searches for primary key.
     */
    ...
    {
        return array_get($this->relationship->def, $def_name);
    }
     ...
     *
     * @param string $table_name name of relationship table to inspect
     *
     * @return bool|string
     */
