...
function construct_where(&$query_obj, $table = '', $module = null)
{
	...
	foreach ($query_obj['conditions'] as $condition) {
		...
		if ($condition['name'] == 'email1' or $condition['name'] == 'email2') {
			...
			$email1_condition = " {$table}id in ( SELECT  er.bean_id AS id FROM email_addr_bean_rel er, "."email_addresses ea WHERE ea.id = er.email_address_id "."AND ea.deleted = 0 AND er.deleted = 0 AND er.bean_module = '{$module}' AND email_address_caps LIKE '%{$email1_value}%' )";
