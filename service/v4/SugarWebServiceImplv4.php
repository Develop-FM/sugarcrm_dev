...
class SugarWebServiceImplv4 extends SugarWebServiceImplv3_1 {
    ...
    public function login($user_auth, $application, $name_value_list = array()){
        Log::info("Begin: SugarWebServiceImpl->login({$user_auth['user_name']}, $application, ". print_r($name_value_list, true) .")");
        ...
        if($success)
        {
            ...
            Log::info('End: SugarWebServiceImpl->login - successful login');
        ...
        self::$helperObject->setFaultObject($error);
        Log::error('End: SugarWebServiceImpl->login - failed login');
    }
    ...
    */
    function get_entry_list($session, $module_name, $query, $order_by,$offset, $select_fields, $link_name_to_fields_array, $max_results, $deleted, $favorites = false ){
        ...
        if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', $module_name, 'read', 'no_access', $error)) {
            Log::error('End: SugarWebServiceImpl->get_entry_list - FAILED on checkSessionAndModuleAccess');
            return;
        } // if
        ...
        if (!self::$helperObject->checkACLAccess($seed, 'list', $error, 'no_access')) {
            Log::error('End: SugarWebServiceImpl->get_entry_list - FAILED on checkACLAccess');
            return;
        } // if
        ...
        Log::info('End: SugarWebServiceImpl->get_entry_list - SUCCESS');
    ...
    function get_module_layout($session, $a_module_names, $a_type, $a_view,$acl_check = TRUE, $md5 = FALSE){
    	Log::info('Begin: SugarWebServiceImpl->get_module_layout');
        ...
        foreach ($a_module_names as $module_name)
        {
            if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', $module_name, 'read', 'no_access', $error))
            {
                Log::error("End: SugarWebServiceImpl->get_module_layout for $module_name - FAILED on checkSessionAndModuleAccess");
                continue;
            }
        ...
        Log::info('End: SugarWebServiceImpl->get_module_layout ->> '.print_r($results,true) );
    ...
    function search_by_module($session, $search_string, $modules, $offset, $max_results,$assigned_user_id = '', $select_fields = array(), $unified_search_only = TRUE, $favorites = FALSE){
    	...
    	if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', '', '', '', $error)) {
    		$error->set_error('invalid_login');
    		Log::error('End: SugarWebServiceImpl->search_by_module - FAILED on checkSessionAndModuleAccess');
    		return;
    	}
    	global $current_user;
    	if($max_results > 0){
    ...
    {
        Log::info('Begin: SugarWebServiceImpl->oauth_request_token');
        ...
        Log::info('End: SugarWebServiceImpl->oauth_request_token');
        return $result;
    }
    ...
    {
        Log::info('Begin: SugarWebServiceImpl->oauth_access_token');
        require_once "include/SugarOAuthServer.php";
        try {
        ...
        Log::info('End: SugarWebServiceImpl->oauth_access_token');
        return $result;
    }

    public function oauth_access($session='')
    {
        Log::info('Begin: SugarWebServiceImpl->oauth_access');
        $error = new SoapError();
    	$output_list = array();
    	if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', '', '', '', $error)) {
    		$error->set_error('invalid_login');
    		Log::info('End: SugarWebServiceImpl->oauth_access');
    		$result = $error;
    	} else {
    ...
    {
        Log::info('Begin: SugarWebServiceImpl->job_queue_next');
        $error = new SoapError();
        if (! self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', '', 'read', 'no_access',  $error)) {
            Log::info('End: SugarWebServiceImpl->job_queue_next denied.');
            return;
        }
        ...
        Log::info('End: SugarWebServiceImpl->job_queue_next');
    ...
    {
        Log::info('Begin: SugarWebServiceImpl->job_queue_cycle');
        $error = new SoapError();
        if (! self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', '', 'read', 'no_access',  $error)) {
            Log::info('End: SugarWebServiceImpl->job_queue_cycle denied.');
            return;
        }
        require_once 'include/SugarQueue/SugarJobQueue.php';
        $queue = new SugarJobQueue();
        $queue->cleanup();
        $queue->runSchedulers();
        Log::info('End: SugarWebServiceImpl->job_queue_cycle');
    ...
    {
        Log::info('Begin: SugarWebServiceImpl->job_queue_run');
        $error = new SoapError();
        if (! self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', '', 'read', 'no_access',  $error)) {
            Log::info('End: SugarWebServiceImpl->job_queue_run denied.');
            return;
        }
        Log::debug('Starting job $jobid execution as $clientid');
        require_once 'modules/SchedulersJobs/SchedulersJob.php';
        $result = SchedulersJob::runJobId($jobid, $clientid);
        Log::info('End: SugarWebServiceImpl->job_queue_run');
        if($result === true) {
