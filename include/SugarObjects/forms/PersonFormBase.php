...
abstract class PersonFormBase extends FormBase
{
     ...
     *
     * @param array $rows Array of duplicate row data
     *
     * @return string $form The HTML form data
     */
    function buildTableForm($rows)
    {
        global $action, $mod_strings, $app_strings;
        ...
        if ($action != 'ShowDuplicates') {
            ...
            $duplicateLabel = string_format($app_strings['MSG_DUPLICATE'], [
                strtolower($this->objectName),
                $this->moduleName
            ]);
            ...
            $duplicateLabel = string_format($app_strings['MSG_SHOW_DUPLICATES'], [
                strtolower($this->objectName),
                $this->moduleName
            ]);
     ...
     * checkForDuplicates
     *
     * This function is used to locate any duplicate Leads that may be created when a new Lead is saved.  It is called
     * from the handleSave method of this class.
     *
     * @param string $prefix value of any prefix to the form input names
     *
     * @return array of matching Leads entries found; null if none found
     */
    function checkForDuplicates($prefix = '')
    {
        ...
        if (! empty($emailStr)) {
            ...
            $query    = 'SELECT DISTINCT er.bean_id AS id FROM email_addr_bean_rel er, '.'email_addresses ea WHERE ea.id = er.email_address_id '.'AND ea.deleted = 0 AND er.deleted = 0 AND er.bean_module = \''.$this->moduleName.'\' '.'AND email_address_caps IN ('.$emailStr.')';
