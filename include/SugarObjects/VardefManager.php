...
class VardefManager
{
    ...
    static function saveCache($module, $object, $additonal_objects = [])
    {
        ...
        $out = "<?php \n \$GLOBALS[\"dictionary\"][\"".$object."\"]=".var_export_helper($GLOBALS['dictionary'][$object]).";";
     ...
     *
     * @param string $module_dir  the module_dir to clear, if not specified then clear vardef cache for all modules.
     ...
     * @param array  $additional_search_paths an array which allows a consumer to pass in additional vardef locations
     *                                        to search
     * @param array  $params
     *
     * @param bool   $cacheCustom
     */
    static function refreshVardefs($module, $object, $additional_search_paths = null, $cacheCustom = true, $params = [])
    {
    ...
    public static function getLinkFieldForRelationship($module, $object, $relName)
    {
        ...
        if (sizeof($matches) == 1) {
            $results = $matches[0];
        } else { //For relationships where both sides are the same module, more than one link will be returned
            $results = $matches;
        }
    ...
    /**
     * This method ensures that the account_name relationships are set to always be required if the configuration file
     * specifies so.  For more information on this require_accounts parameter, please see the administrators guide or
     * go to the developers.sugarcrm.com website to find articles relating to the use of this field.
     *
     * @param array $vardef The vardefs of the module to apply the account_name field requirement to
     *
     * @return array The vardefs of the module with the updated required setting based on the system configuration
     */
