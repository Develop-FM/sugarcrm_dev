...
class SugarEmailAddress extends SugarBean
{
        ...
        if (! empty($this->addresses)) {
            ...
            foreach ($this->addresses as $address) {
                if (! empty($address['email_address'])) {
                    ...
                    $emailId = $this->AddUpdateEmailAddress($address['email_address'], $address['invalid_email'], $address['opt_out'], $emailId);// this will save the email address if not found
     ...
     *                        in case a "email has changed" WorkFlow has triggered - hack to allow workflow-induced
     *                        changes to propagate to the new SugarEmailAddress - see bug 39188
     *
    ...
    public function AddUpdateEmailAddress($addr, $invalid = 0, $opt_out = 0, $id = null)
    {
        ...
        if (! empty($current_email['id']) && isset($this->stateBeforeWorkflow[$current_email['id']])) {
            if ($current_email['invalid_email'] != $invalid || $current_email['opt_out'] != $opt_out) {
        ...
        if (! empty($duplicate_email['id'])) {
            ...
            if ($duplicate_email['invalid_email'] != $new_invalid || $duplicate_email['opt_out'] != $new_opt_out || (trim($duplicate_email['email_address']) != $address)) {
                $upd_q = 'UPDATE '.$this->table_name.' '.'SET email_address=\''.$address.'\', '.'invalid_email='.$new_invalid.', '.'opt_out='.$new_opt_out.', '.'date_modified='.$this->db->now().' '.'WHERE id=\''.$this->db->quote($duplicate_email['id']).'\'';
    ...
    function getEmailAddressWidgetDetailView($focus, $tpl = '')
    {
        ...
        foreach ($prefillData as $addressItem) {
            ...
            $assign[] = [
                'key'     => $key,
                'address' => $current_user->getEmailLink2($addressItem['email_address'], $focus).$addressItem['email_address']."</a>"
            ];
        }
