...
function make_sugar_config(&$sugar_config)
{
	...
	$sugar_config = [
		...
		'chartEngine'                             => 'Jit',
		'date_formats'                            => empty($dateFormats) ? [
			'Y-m-d' => '2010-12-23',
			'd-m-Y' => '23-12-2010',
			'm-d-Y' => '12-23-2010',
			'Y/m/d' => '2010/12/23',
		...
		'languages'                               => empty($languages) ? ['en_us' => 'English (US)'] : $languages,
		...
		'name_formats'                            => empty($nameFormats) ? [
			's f l'  => 's f l',
			'f l'    => 'f l',
			's l'    => 's l',
			'l, s f' => 'l, s f',
		...
		'resource_management'                     => [
			...
			'special_query_modules' => ['Reports', 'Export', 'Import', 'Administration', 'Sync'],
			'default_limit'         => 1000,
		],
		...
		// this must be set!!
		'showDetailData'                          => true,
		// if true, read-only ACL fields will still appear on EditViews as non-editable
		'showThemePicker'                         => true,
		'sugar_version'                           => empty($sugar_version) ? 'unknown' : $sugar_version,
		'time_formats'                            => empty($timeFormats) ? [
			'H:i'  => '23:00',
			'h:ia' => '11:00 pm',
			'h:iA' => '11:00PM',
			'H.i'  => '23.00',
		...
		'upload_badext'                           => empty($upload_badext) ? [
			'php',
			'php3',
			'php4',
			'php5',
...
function get_sugar_config_defaults()
{
	...
	$sugar_config_defaults = [
		...
		// use RFC2368 spec unless we have a noncompliant email client
 ...
 * @param string $user_id          String value to specify a particular user id value (searches the id column of users
 *                                 table), blank by default
 * @param bool   $use_real_name    Boolean value indicating whether or not results should include the full name or just
 *                                 user_name, false by default
 * @param String $user_name_filter String value indicating the user_name filter (searches the user_name column of users
 *                                 table) to optionally search with, blank by default
 * @param string $portal_filter    String query filter for portal users (defaults to searching non-portal users),
 *                                 change to blank if you wish to search for all users including portal users
 * @param bool   $from_cache       Boolean value indicating whether or not to use the get_register_value function for
 *                                 caching, true by default
 *
 * @return array Array of users matching the filter criteria that may be from cache (if similar search was previously
 *               run)
 */
function get_user_array(
	$add_blank = true, $status = "Active", $user_id = '', $use_real_name = false, $user_name_filter = '',
...
/**
 * This function retrieves an application language file and returns the array of strings included in the
 * $app_list_strings var.
 *
...
function return_app_list_strings_language($language)
{
	...
	foreach ($langs as $lang) {
		if (file_exists("custom/application/Ext/Language/$lang.lang.ext.php")) {
			...
			Log::info("Found extended language file: $lang.lang.ext.php");
		}
		if (file_exists("custom/include/language/$lang.lang.php")) {
			include("custom/include/language/$lang.lang.php");
			Log::info("Found custom language file: $lang.lang.php");
		}
	}

	if (! isset($app_list_strings)) {
		Log::fatal("Unable to load the application language file for the selected language ($language) or the default language ($default_language) or the en_us language");
 ...
 * $GLOBALS['app_list_strings']['$key'] = $value, so we have to delete the original ones in app_list_strings and relace
 * it with the custom ones.
 *
...
function return_application_language($language)
{
	...
	if (! isset($app_strings)) {
		Log::fatal("Unable to load the application language strings");
...
function return_module_language($language, $module, $refresh = false)
{
	...
	if (empty($module)) {
		...
		Log::warn("Variable module is not in return_module_language ".var_export_helper($stack));
	...
	if ($language != $sugar_config['default_language']) {
		$loaded_mod_strings = sugarLangArrayMerge(LanguageManager::loadModuleLanguage($module, $sugar_config['default_language'], $refresh), $loaded_mod_strings);
	}
...
function return_mod_list_strings_language($language, $module)
{
	...
	if (! isset($mod_list_strings)) {
		Log::fatal("Unable to load the application list language file for the selected language($language) or the default language($default_language) for module({$module})");
 ...
 * It looks for the variable in the $_REQUEST array.  If it is set and is not "" it will create a where clause out of
 * it.
 *
 ...
 * @param $SQL_name       - [Optional] If specified, this is the SQL column name that is used.  If not specified, the
 *                        $variable_name is used as the SQL_name. Portions created by SugarCRM are Copyright (C)
 *                        SugarCRM, Inc. All Rights Reserved. Contributor(s): ______________________________________..
 */
...
{
	Log::deprecated("get_admin_modules_for_user() is deprecated as of 6.2.2 and may disappear in the future, use Users->getDeveloperModules() instead");
...
/**
 * Create HTML to display select options in a dropdown list.  To be used inside
 * of a select statement in a form.   This method expects the option list to have keys and values.  The keys are the
 * ids.  The values are the display strings. param $option_list - the array of strings to that contains the option list
 * param $selected - the string which contains the default value Portions created by SugarCRM are Copyright (C)
 * SugarCRM, Inc. All Rights Reserved. Contributor(s): ______________________________________..
 */
 ...
 * Create HTML to display select options in a dropdown list.  To be used inside
 * of a select statement in a form.   This method expects the option list to have keys and values.  The keys are the
 * ids.  The values are the display strings. param $label_list - the array of strings to that contains the option list
 * param $key_list - the array of strings to that contains the values list param $selected - the string which contains
 * the default value Portions created by SugarCRM are Copyright (C) SugarCRM, Inc. All Rights Reserved. Contributor(s):
...
function get_select_options_with_id_separate_key($label_list, $key_list, $selected_key, $massupdate = false)
{
	...
	foreach ($key_list as $option_key => $option_value) {
		...
		if (($option_key != '' && $selected_key == $option_key) || ($option_key == '' && (($selected_key == '' && ! $massupdate) || $selected_key == '__SugarMassUpdateClearField__')) || (is_array($selected_key) && in_array($option_key, $selected_key))) {
...
/**
 * Very cool algorithm for sorting multi-dimensional arrays.  Found at
 * http://us2.php.net/manual/en/function.array-multisort.php Syntax: $new_array = array_csort($array [, 'col1' [,
 * SORT_FLAG [, SORT_FLAG]]]...); Explanation: $array is the array you want to sort, 'col1' is the name of the column
 * you want to sort, SORT_FLAGS are : SORT_ASC, SORT_DESC, SORT_REGULAR, SORT_NUMERIC, SORT_STRING you can repeat the
 * 'col',FLAG,FLAG, as often you want, the highest prioritiy is given to the first - so the array is sorted by the last
 * given column first, then the one before ... Example: $array = array_csort($array,'town','age',SORT_DESC,'name');
 * Portions created by SugarCRM are Copyright (C) SugarCRM, Inc. All Rights Reserved. Contributor(s):
 * ______________________________________..
 */
...
function parse_calendardate($local_format)
{
	...
	return str_replace(["y", "ￄ1�7", "a", "j"], ["Y", "Y", "Y", "d"], $calendar_format);
}
...
function clean_string($str, $filter = "STANDARD", $dieOnBadData = true)
{
	...
	if (preg_match($filters[$filter], $str)) {
		if (isset($GLOBALS['log']) && is_object($GLOBALS['log'])) {
			Log::fatal("SECURITY[$filter]: bad data passed in; string: {$str}");
		}
		if ($dieOnBadData) {
...
{
	return preg_replace_callback('|[^A-Za-z0-9\-]|', create_function(// single quotes are essential here,
		// or alternative escape all $ as \$
			'$matches', 'return ord($matches[0]);'), $string);
}
...
function clone_history(&$db, $from_id, $to_id, $to_type)
{
	...
	$tables = ['calls' => 'Call', 'meetings' => 'Meeting', 'notes' => 'Note', 'tasks' => 'Task'];
...
function get_emails_by_assign_or_link($params)
{
	...
	if (empty($bean->$relation)) {
		Log::error("Bad relation '$relation' for bean '{$bean->object_name}' id '{$bean->id}'");

		return [];
	}
	$rel_module             = $bean->$relation->getRelatedModuleName();
	$rel_join               = $bean->$relation->getJoin([
...
function get_bean_select_array(
	$add_blank = true, $bean_name, $display_columns, $where = '', $order_by = '', $blank_is_none = false
) {
	global $beanFiles;
...
function check_iis_version($sys_iis_version = '')
{
	...
	$supported_iis_versions   = ['6.0', '7.0',];
	$unsupported_iis_versions = [];
	$invalid_iis_versions     = ['5.0',];
...
function pre_login_check()
{
	...
	if (! empty($action) && $action == 'Login') {
		...
		if (! empty($login_error)) {
			...
			$login_error             = str_replace(["&lt;pre&gt;", "&lt;/pre&gt;", "\r\n", "\n"], "<br>", $login_error);
...
/**
 * In order to have one place to obtain the proper object name. aCase for example causes issues throughout the
 * application.
 *
		...
		if (preg_match('/<h.>([^<]+)<\/h.>/', $parsedInfo[$i], $match)) {
			...
			foreach ($parsedInfo2 AS $vOne) {
				...
				if (preg_match($vPat3, $vOne, $match)) { // 3cols
					$returnInfo[$vName][trim($match[1])] = [trim($match[2]), trim($match[3])];
...
function getJavascriptSiteURL()
{
	...
	Log::debug("getJavascriptSiteURL(), site_url=".$site_url);
...
function array_depth($array, $depth_count = -1, $depth_array = [])
{
	$depth_count++;
	if (is_array($array)) {
...
function getStudioIcon(
	$iconFileName = '', $altFileName = '', $width = '48', $height = '48', $align = 'baseline', $alt = ''
) {
 ...
 * For some user using mssql without FreeTDS, they may store multibyte charaters in varchar using latin_general
 * collation. It cannot store so many mutilbyte characters, so we need to use strlen. The varchar in MySQL, Orcale, and
 * nvarchar in FreeTDS, we can store $length mutilbyte charaters in it. we need mb_substr to keep more info.
...
function create_export_query_relate_link_patch($module, $searchFields, $where)
{
	...
	$ret_array = ['where' => $where, 'join' => isset($join['join']) ? $join['join'] : ''];

	return $ret_array;
}

/**
 * We need to clear all the js cache files, including the js language files  in serval places in MB. So I extract them
 * into a util function here.
...
function verify_image_file($path, $jpeg = false)
{
	if (function_exists('imagepng') && function_exists('imagejpeg') && function_exists('imagecreatefromstring')) {
		...
		if (preg_match("/<(\?php|html|!doctype|script|body|head|plaintext|table|img |pre(>| )|frameset|iframe|object|link|base|style|font|applet|meta|center|form|isindex)/i", $data, $m)) {
			Log::fatal("Found {$m[0]} in $path, not allowing upload");
...
{
	$supportedExtensions = ['jpg' => 'image/jpeg', 'jpeg' => 'image/jpeg'];
	if (! $jpeg_only) {
	...
	if (substr_count('..', $path) > 0 || ($ext !== $path && ! isset($supportedExtensions[strtolower($ext)])) || ! in_array($filetype, array_values($supportedExtensions))) {
		return false;
	}
...
function sql_like_string($str, $like_char, $wildcard = '%', $appendWildcard = true)
{
	...
	if (isset($GLOBALS['sugar_config']['search_wildcard_char']) && strlen($GLOBALS['sugar_config']['search_wildcard_char']) == 1) {
		$wildcard = $GLOBALS['sugar_config']['search_wildcard_char'];
	}

	// add wildcard at the beginning of the search string
	if (isset($GLOBALS['sugar_config']['search_wildcard_infront']) && $GLOBALS['sugar_config']['search_wildcard_infront'] == true) {
...
function get_help_url(
	$send_edition = '', $send_version = '', $send_lang = '', $send_module = '', $send_action = '', $dev_status = '',
	$send_key = '', $send_anchor = ''
) {
	global $sugar_config;
...
function assignConcatenatedValue(SugarBean $bean, $fieldDef, $value)
{
	...
	if (count($valueParts) == 1 && $fieldDef['db_concat_fields'] == ['first_name', 'last_name']) {
