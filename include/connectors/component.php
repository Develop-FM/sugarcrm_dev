...
class component
{
	 ...
	 * @param SugarBean|null $bean   SugarBean instance to load values into
	 *
	 * @return array|null
	 * @throws Exception Thrown if results could not be loaded into bean
	 */
	public function fillBean($args = [], $module = null, SugarBean $bean = null)
	{
		$result = null;
		if (! is_null($bean)) {
	 ...
	 * loadBean configuration mapping
	 *
	 * @param array $args   Array of arguments to pass into getItem
	 * @param string $module String value of the module to map bean to
	 * @param array $beans   Array to load SugarBean intances into
	 *
	 * @return array
	...
	public function fillBeans($args = [], $module = null, $beans = [])
	{
		...
		if (empty($args)) {
			Log::fatal($GLOBALS['app_strings']['ERR_MISSING_MAPPING_ENTRY_FORM_MODULE']);
			throw new Exception($GLOBALS['app_strings']['ERR_MISSING_MAPPING_ENTRY_FORM_MODULE']);
		}

		require_once(DOCROOT.'include/connectors/filters/FilterFactory.php');
	 ...
	 * @return array
	 */
	public function mapInput(array $inputData, $module)
	{
		$input_params = [];
		$map          = $this->getMapping();
	 ...
	 * @throws Exception
	 */
	public function mapOutput(SugarBean $bean, $result)
	{
		$map     = $this->getMapping();
		...
		if (empty($mapping) || (count($mapping) == 1 && isset($mapping['id']))) {
			Log::error($GLOBALS['mod_strings']['ERROR_NO_DISPLAYABLE_MAPPED_FIELDS']);
	 ...
	 * @return mixed
	 */
	private function getFieldValue(SugarBean $bean, array $result, $source_field)
	{
	...
	{
		$this->_source->saveConfig();
	}

	public function loadConfig()
	{
		$this->_source->loadConfig();
	}
