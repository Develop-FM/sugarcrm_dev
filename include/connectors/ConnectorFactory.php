...
class ConnectorFactory
{
    ...
    public static function getInstance($source_name)
    {
        if (empty(self::$source_map[$source_name])) {
            ...
            require_once(DOCROOT.'include/connectors/sources/SourceFactory.php');
    ...
    public static function loadClass($class, $type)
    {
        ...
        if (file_exists($path = DOCROOT."custom/modules/Connectors/connectors/{$type}/{$dir}/$file")) {
            require_once($path);
        } else if (file_exists($path = DOCROOT."modules/Connectors/connectors/{$type}/{$dir}/$file")) {
            require_once($path);
        } else if (file_exists($path = DOCROOT."connectors/{$type}/{$dir}/$file")) {
            require_once($path);
        }
    }
}