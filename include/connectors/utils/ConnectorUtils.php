...
class ConnectorUtils
{
     ...
     *
     * @return array $searchdefs Array of the search definitions
     */
    public static function getSearchDefs($refresh = false)
    {
        if ($refresh || ! file_exists(DOCROOT.'custom/modules/Connectors/metadata/searchdefs.php')) {

            require(DOCROOT.'modules/Connectors/metadata/searchdefs.php');

            if (! file_exists(DOCROOT.'custom/modules/Connectors/metadata')) {
                mkdir_recursive(DOCROOT.'custom/modules/Connectors/metadata');
            }
        ...
        require(DOCROOT.'custom/modules/Connectors/metadata/searchdefs.php');
     ...
     *
     * @param array $filter_sources Array optional Array value of sources to only use
     *
     * @return array $mergedefs Array of the merge definitions
     */
    public static function getViewDefs($filter_sources = [])
    {
    ...
    public static function getMergeViewDefs($refresh = false)
    {
        if ($refresh || ! file_exists('custom/modules/Connectors/metadata/mergeviewdefs.php')) {
            ...
            if (! write_array_to_file('viewdefs', $view_defs, 'custom/modules/Connectors/metadata/mergeviewdefs.php')) {
                Log::fatal("Cannot write file custom/modules/Connectors/metadata/mergeviewdefs.php");
        ...
        if ($refresh || ! file_exists($src4)) {
            ...
            if (file_exists($src4)) {
                ...
                if (! isset($connectors) || ! is_array($connectors)) {
                    $connectors = [];
                    $err_str    = string_format($GLOBALS['app_strings']['ERR_CONNECTOR_NOT_ARRAY'], [$src4]);
                    Log::error($err_str);
                }
    ...
    public static function saveConnectors($connectors, $toFile = '')
    {
        ...
        if (! write_array_to_file('connectors', $connectors, $toFile)) {
            ...
            Log::fatal("Cannot write sources to file");
    ...
    private static function getSources($directory = 'modules/Connectors/connectors/sources')
    {
        if (file_exists($directory)) {
            ...
            foreach ($files as $file) {
                ...
                $sources_ordering[$source['id']] = ['order' => $order, 'source' => $source];
            }
     ...
     *
     * @return boolean $enabled boolean value indicating whether or not the module is enabled to be serviced by the
     *                 connector module
     */
    ...
    /**
     * When a module has all of the sources removed from it we do not properly remove it from the viewdefs. This
    ...
    public function removeHoverField(&$viewdefs, $module)
    {
        ...
        if (! $metaParser->hasMultiplePanels($viewdefs[$module]['DetailView']['panels'])) {
            ...
            if (! empty($keys) && count($keys) != 1) {
                $viewdefs[$module]['DetailView']['panels'] = ['default' => $viewdefs[$module]['DetailView']['panels']];
            }
        }
                        ...
                        if (is_array($field)) {
                            if (! empty($viewdefs[$module]['DetailView']['panels'][$panel_id][$row_id][$field_id]['displayParams'])) {
                                ...
                                $field['displayParams']                                                   = [
                                    ...
                                    'module'           => $module,
                                    'connectors'       => [0 => $source_id]
                                ];
                                $viewdefs[$module]['DetailView']['panels'][$panel_id][$row_id][$field_id] = $field;
                            }
                        } else {
                            $viewdefs[$module]['DetailView']['panels'][$panel_id][$row_id][$field_id] = [
                                'name'          => $field,
                                'displayParams' => [
                                    'enableConnectors' => true,
                                    'module'           => $module,
     ...
     * @param string $source_id
     *
     * @return bool True if field was added; false otherwise
     */
     ...
     *
     * @param array        $displayParams Array value of display parameters passed from the SugarField code
     * @param Sugar_Smarty $smarty        The Smarty object from the calling smarty code
     *
    ...
    public static function getConnectorStrings($source_id, $language = '')
    {
        ...
        if (file_exists("custom/modules/Connectors/connectors/sources/{$dir}/language/{$lang}")) {
            ...
            return ! empty($connector_strings) ? $connector_strings : [];
            ...
            Log::error("Unable to locate language string file for source {$source_id}");
     ...
     *
     * @param string $source_id         String value of the connector id to write language strings for (e.g.,
     *                                  ext_soap_marketo)
     * @param string $connector_strings array value of the connector_strings
     * @param string $language          optional String value for the language to use (defaults to
    ...
    public static function setConnectorStrings($source_id, $connector_strings, $language = '')
    {
        ...
        if (! write_array_to_file("connector_strings", $connector_strings, "custom/modules/Connectors/connectors/sources/{$dir}/language/{$lang}")) {
            //Log error and return empty array
            Log::fatal("Cannot write connectory_strings to file custom/modules/Connectors/connectors/sources/{$dir}/language/{$lang}");
