...
class FormatterFactory
{
     ...
     *
     * @param string $source_name    The data source name to retreive formatter for
     * @param string $formatter_name Optional formatter name to use
     *
     * @return default_formatter $instance The formatter instance
     */
    public static function getInstance($source_name, $formatter_name = '')
    {
        require_once(DOCROOT.'include/connectors/formatters/default/formatter.php');
        ...
        if (empty(self::$formatter_map[$key])) {
            ...
            if (file_exists(DOCROOT."modules/Connectors/connectors/formatters/{$dir}/{$file}.php") || file_exists(DOCROOT."custom/modules/Connectors/connectors/formatters/{$dir}/{$file}.php")) {
            ...
            if (file_exists($path = DOCROOT."custom/modules/Connectors/connectors/formatters/{$dir}/tpls/{$file}.tpl")) {
                $formatter->setTplFileName($path);
            } else if (file_exists($path = DOCROOT."modules/Connectors/connectors/formatters/{$dir}/tpls/{$file}.tpl")) {
                $formatter->setTplFileName($path);
            }
