...
class SugarController
{
     ...
     * @var array
     */
    protected $action_remap = ['index' => 'listview'];
     ...
     * @var array
     */
    public static $action_case_file = [
        'editview'   => 'EditView',
    ...
    {
        if (empty($module) && $this->request->has('module')) {
    ...
    protected function post_delete()
    {
        if (empty($_REQUEST['return_url'])) {
            $return_module = isset($_REQUEST['return_module']) ? $_REQUEST['return_module'] : $GLOBALS['sugar_config']['default_module'];
            $return_action = isset($_REQUEST['return_action']) ? $_REQUEST['return_action'] : $GLOBALS['sugar_config']['default_action'];
    ...
    */
    ...
    protected function action_callmethoddashlet()
    {
        if (! empty($_REQUEST['id'])) {
            ...
            if (! empty($dashletDefs[$id])) {
                ...
                $dashlet = new $dashletDefs[$id]['className']($id, (isset($dashletDefs[$id]['options']) ? $dashletDefs[$id]['options'] : []));
    ...
    public function checkEntryPointRequiresAuth($entryPoint)
    {
        ...
        if (isset($this->entry_point_registry[$entryPoint]['auth']) && ! $this->entry_point_registry[$entryPoint]['auth']) {
            return false;
        }
    ...
    protected function callLegacyCode()
    {
        ...
        if ((file_exists('modules/'.$this->module.'/'.$file.'.php') && ! file_exists('modules/'.$this->module.'/views/view.'.$action.'.php')) || (file_exists('custom/modules/'.$this->module.'/'.$file.'.php') && ! file_exists('custom/modules/'.$this->module.'/views/view.'.$action.'.php'))) {
