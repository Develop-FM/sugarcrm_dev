...
class ViewFactory
{
    ...
    public static function loadView(
        $type = 'default', $module, SugarBean $bean = null, array $view_object_map = [], $target_module = ''
    ) {
        ...
        if (! empty($target_module)) {
            ...
            } else if (file_exists($path = DOCROOT."modules/{$target_module}/views/view.{$type}.php")) {
                $view = ViewFactory::_buildFromFile($path, $bean, $view_object_map, $type, $target_module);
            }
        }
        ...
        if (! isset($view)) {
            ...
            } else if (file_exists($path = DOCROOT."modules/{$module}/views/view.{$type}.php")) {
                $view = ViewFactory::_buildFromFile($path, $bean, $view_object_map, $type, $module);
            } else if (file_exists($path = DOCROOT."custom/include/MVC/View/views/view.{$type}.php")) {
                $view = ViewFactory::_buildFromFile($path, $bean, $view_object_map, $type, $module);
            } else {
     ...
     *
     * @param SugarView $view
     * @param atring    $type
     */
    protected static function _loadConfig(SugarView & $view, $type)
    {
        $view_config_custom    = [];
        $view_config_module    = [];
        ...
        if (! $view_config) {
            if (file_exists($path = DOCROOT."custom/modules/{$view->module}/views/{$config_file_name}")) {
                require_once($path);
                $view_config_custom = $view_config;
            }

            if (file_exists($path = DOCROOT."modules/{$view->module}/views/{$config_file_name}")) {
                require_once($path);
                $view_config_module = $view_config;
            }

            if (file_exists($path = DOCROOT."custom/include/MVC/View/views/{$config_file_name}")) {
                require_once($path);
                $view_config_root_cstm = $view_config;
            }

            if (file_exists($path = DOCROOT."include/MVC/View/views/{$config_file_name}")) {
                require_once($path);
     ...
     * @return SugarView
     */
    protected static function _buildFromFile($file, SugarBean &$bean = null, $view_object_map, $type, $module)
    {
        require_once($file);
        ...
        if (file_exists($path = DOCROOT.'custom/include/MVC/View/SugarView.php')) {
            require_once($path);
     ...
     * @param SugarBean $bean            = the bean to pass to the view
     * @param array     $view_object_map - the array which holds obejcts to pass between the
     *                                   controller and the view.
     *
     * @return SugarView
     */
    protected static function _buildClass($class, SugarBean $bean = null, array $view_object_map)
    {
        $view = new $class();
