 ...
 *
 * Main features are to list available external API's by supported features, modules and which ones have access for the
 * user.
 * @api
 */
class ExternalAPIFactory
{
    ...
    public static function loadFullAPIList($forceRebuild = false, $ignoreDisabled = false)
    {
        ...
        $baseDirList = ['include/externalAPI/', 'custom/include/externalAPI/'];
    ...
    /**
     * This will hand back an initialized class for the requested external API, it will also load in the external API
     * password information into the bean.
     *
     * @param string $apiName    The name of the requested API ( known API's can be listed by the listAPI() call )
     * @param bool   $ignoreAuth Ignore authentication requirements (optional)
     *
    ...
    /**
     * Lists the available API's for a module or all modules, and possibly ignoring if the user has auth information
     * for that API even if it is required
     *
