 ...
 * @return string
 */
function var_export_helper($var, $indent = "")
{
    switch (gettype($var)) {
        case "string":
        ...
        case "array":
            ...
            foreach ($var as $key => $value) {
                $r[] = "$indent    ".($indexed ? "" : var_export_helper($key)." => ").var_export_helper($value, "$indent    ");
            }
 ...
 *
 * @param string $array_name - a String containing the name of an array.
 * @param string $value_name - a String containing the name of a variable in the array.
 * @param string $value      - a String containing the associated value with $value_name.
 *
 * @return string. Example - override_value_to_string($name, 'b', 1) = '$name['b'] = 1;'
 */
...
/**
 * Given an array and key names, return a string in the form of $array_name[$key_name[0]][$key_name[1]]... = $value
 * recursively.
 *
 * @param array  $key_names  - array of keys
 * @param string $array_name - name of the array
 * @param mixed  $value      -value of the array
 * @param bool   $eval       - evals the generated string if true, note that the array name must be in the global
 *                           space!
 *
 * @return string : example - string $array_name['a']['b']['c'][.] = 'hello'
 */
function override_value_to_string_recursive($key_names, $array_name, $value, $eval = false)
{
    if ($eval) {
        return eval("\${$array_name}".override_recursive_helper($key_names, $array_name, $value));
    } else {
...
class SugarArray extends ArrayObject
{
    ...
    private function _getRecursive($raw_config, $children, $default)
    {
        if ($raw_config === $default) {
            ...
            return isset($raw_config[$next_key]) ? $this->_getRecursive($raw_config[$next_key], $children, $default) : $default;
        }
    }
}
