...
class SugarTheme
{
     ...
     *
     * @param string $defaults string defaults for the current theme
     */
    ...
    public function __destruct()
    {
        ...
        if ($this->_clearCacheOnDestroy) {
            ...
            if (count($this->_jsCache) != $this->_initialCacheSize['jsCache'] || count($this->_cssCache) != $this->_initialCacheSize['cssCache'] || count($this->_imageCache) != $this->_initialCacheSize['imageCache'] || count($this->_templateCache) != $this->_initialCacheSize['templateCache']) {
                sugar_file_put_contents("$cachedir/pathCache.php", serialize([
            ...
            if (count($this->_spriteCache) != $this->_initialCacheSize['spriteCache']) {
                sugar_file_put_contents("$cachedir/spriteCache.php", serialize($this->_spriteCache));
            }
        }
    }
     ...
     *
     * @param string $color string optional, specifies the css color file to use if the theme supports it; defaults to
     *                      cookie value or theme default
     * @param string $font  string optional, specifies the css font file to use if the theme supports it; defaults to
     *                      cookie value or theme default
     *
     * @return string HTML code
     */
    public function getCSS($color = null, $font = null)
    {
        ...
        if (! empty($GLOBALS['sugar_config']['use_sprites']) && $GLOBALS['sugar_config']['use_sprites']) {
            ...
            // system wide sprites
            if (file_exists("storage/cache/sprites/default/sprites.css")) {
                $html .= '<link rel="stylesheet" type="text/css" href="'.getJSPath('storage/cache/sprites/default/sprites.css').'" />';
            }

            // theme specific sprites
            if (file_exists("storage/cache/sprites/{$this->dirName}/sprites.css")) {
                $html .= '<link rel="stylesheet" type="text/css" href="'.getJSPath('storage/cache/sprites/'.$this->dirName.'/sprites.css').'" />';
            }

            // parent sprites
            if ($this->parentTheme && $parent = SugarThemeRegistry::get($this->parentTheme)) {
                if (file_exists("storage/cache/sprites/{$parent->dirName}/sprites.css")) {
                    $html .= '<link rel="stylesheet" type="text/css" href="'.getJSPath('storage/cache/sprites/'.$parent->dirName.'/sprites.css').'" />';
                }
            }

            // repeatable sprites
            if (file_exists("storage/cache/sprites/Repeatable/sprites.css")) {
                $html .= '<link rel="stylesheet" type="text/css" href="'.getJSPath('storage/cache/sprites/Repeatable/sprites.css').'" />';
            }
        }
...
EOHTML;
    ...
    public function getTemplate($templateName)
    {
        ...
        } elseif (isset($this->parentTheme) && SugarThemeRegistry::get($this->parentTheme) instanceOf SugarTheme && ($filename = SugarThemeRegistry::get($this->parentTheme)->getTemplate($templateName)) != '') {
     ...
     * @param  string $alt              optional, only used when image contains something useful, i.e. "Sally's profile
     *                                  pic"
     *
    ...
    public function getImage($imageName, $other_attributes = '', $width = null, $height = null, $ext = null, $alt = '')
    {
        ...
        if (! empty($GLOBALS['sugar_config']['use_sprites']) && $GLOBALS['sugar_config']['use_sprites']) {
            ...
            if ($sp = $this->getSpriteMeta($imageName)) {
                ...
                if ((! is_null($width) && $sp['width'] == $width) || (is_null($width)) && (! is_null($height) && $sp['height'] == $height) || (is_null($height))) {
    ...
    public function getLink(
        $url, $title, $other_attributes = '', $img_name = '', $img_other_attributes = '', $img_width = null,
        $img_height = null, $img_alt = '', $img_placement = 'imageonly'
    ) {
        if ($img_name) {
    ...
    public function getImageURL($imageName, $addJSPath = true)
    {
        ...
        if (($filename = $this->_getImageFileName('custom/'.$this->getImagePath().'/'.$imageName)) != '') {
            ...
            $imagePath = $filename;
        } elseif (isset($this->parentTheme) && SugarThemeRegistry::get($this->parentTheme) instanceOf SugarTheme && ($filename = SugarThemeRegistry::get($this->parentTheme)->getImageURL($imageName, false)) != '') {
            $imagePath = $filename;
    ...
    protected function _getImageFileName($imageName)
    {
        ...
        foreach ($this->imageExtensions as $extension) {
            if (isset($pathParts['extension'])) {
                if (($extension != $pathParts['extension']) && sugar_is_file($pathParts['dirname'].'/'.$pathParts['filename'].'.'.$extension)) {
    ...
    public function getCSSURL($cssFileName, $returnURL = true)
    {
        if (isset($this->_cssCache[$cssFileName]) && sugar_is_file(sugar_cached($this->_cssCache[$cssFileName]))) {
            if ($returnURL) {
                return getJSPath("storage/cache/".$this->_cssCache[$cssFileName]);
            } else {
        ...
        if ($returnURL) {
            return getJSPath("storage/cache/".$fullFileName);
        }
    ...
    public function getJSURL($jsFileName, $returnURL = true)
    {
        if (isset($this->_jsCache[$jsFileName]) && sugar_is_file(sugar_cached($this->_jsCache[$jsFileName]))) {
            if ($returnURL) {
                return getJSPath("storage/cache/".$this->_jsCache[$jsFileName]);
            } else {
...
class SugarThemeRegistry
{
     ...
     * @return bool
     */
    public static function add(array $themedef)
    {
     ...
     * from upgrades.  It attempts to look for the Sugar theme and if not found, defaults to return the name of the
     * last theme in the array of available themes loaded.
     *
     * @return string $defaultThemeKey String value of the default theme key to use
     */
