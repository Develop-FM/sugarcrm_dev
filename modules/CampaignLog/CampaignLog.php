...
class CampaignLog extends SugarBean
{
    ...
    function get_list_view_data()
    {
        ...
        if (! (isset($temp_array['TARGET_TYPE']) && $temp_array['TARGET_TYPE'] != '') || ! (isset($temp_array['TARGET_ID']) && $temp_array['TARGET_ID'] != '')) {   //needed values to construct query are empty/null, so return null
            Log::debug("CampaignLog.php:get_list_view_data: temp_array['TARGET_TYPE'] and/or temp_array['TARGET_ID'] are empty, return null");
            $emptyArr = [];
        ...
        if ($temp_array['TARGET_TYPE'] == 'Accounts') {
            ...
            $query = "select first_name, last_name, ".$this->db->concat($table, [
                    'first_name',
                    'last_name'
                ])." name from $table"." where id = ".$this->db->quoted($temp_array['TARGET_ID']);
        }
