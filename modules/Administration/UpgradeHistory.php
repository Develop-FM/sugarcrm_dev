...
class UpgradeHistory extends SugarBean
{
    ...
    var $column_fields = ["id", "filename", "md5sum", "type", "version", "status", "date_entered"];
    ...
    function determineIfUpgrade($id_name, $version)
    {
        ...
        if (empty($result)) {
            ...
            if ($this->is_right_version_greater(explode('.', $temp_version), explode('.', $version), false)) {
                return ['id' => $id, 'version' => $temp_version];
            } else {
                return null;
            }
        }
    ...
    function UninstallAvailable($patch_list, $patch_to_check)
    {
        ...
        foreach ($patch_list as $more_recent_patch) {
            ...
            if ($this->foundConflict($patch_to_check_backup_path, $more_recent_patch_backup_path) && ($more_resent_patch_timestamp >= $patch_to_check_timestamp)) {
                return false;
            }
        }
