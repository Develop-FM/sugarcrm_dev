...
class SugarFeed extends Basic
{
    ...
    {
        $baseDirList = ['modules/'.$module.'/SugarFeeds/', 'custom/modules/'.$module.'/SugarFeeds/'];
    ...
    static function getActiveFeedModules()
    {
        ...
        fwrite($fd, '<'."?php\n\n".'$feedModules = '.var_export_helper($feedModules).';');
        fclose($fd);
    ...
    static function getAllFeedModules()
    {
        ...
        $feedModules = ['UserFeed' => 'UserFeed'];

        $baseDirList = ['modules/', 'custom/modules/'];
        foreach ($baseDirList as $baseDir) {
            if (! file_exists($baseDir)) {
                continue;
            }
    ...
    static function pushFeed(
        $text, $module, $id, $record_assigned_user_id = false, $link_type = false, $link_url = false
    ) {
        $feed = new SugarFeed();
    ...
    static function getLinkTypes()
    {
        ...
        $baseDirs = ['custom/modules/SugarFeed/linkHandlers/', 'modules/SugarFeed/linkHandlers'];
        ...
        fwrite($fd, '<'."?php\n\n".'$linkTypeList = '.var_export_helper($linkTypeList).';');
        fclose($fd);
