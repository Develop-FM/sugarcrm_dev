...
class ViewModulefield extends SugarView
{
    ...
    function fetch($ac = false)
    {
        ...
        $isClone = false;
        if (! empty($this->view_object_map['is_clone']) && $this->view_object_map['is_clone'] && (strcmp($field_name, "name") != 0)   // bug #35767, do not allow cloning of name field
        ) {
            $isClone = true;
        }
        ...
        if (empty($_REQUEST['view_package']) || $_REQUEST['view_package'] == 'studio') {
            ...
            $vardef = (! empty($module->mbvardefs->vardefs['fields'][$field_name])) ? $module->mbvardefs->vardefs['fields'][$field_name] : [];
            if ($isClone) {
                unset($vardef['name']);
            }
        ...
        foreach (["formula", "default", "comments", "help", "visiblityGrid"] as $toEscape) {
            if (! empty($vardef[$toEscape]) && is_string($vardef[$toEscape])) {
                $vardef[$toEscape] = htmlentities($vardef[$toEscape], ENT_QUOTES, 'UTF-8');
            }
        }

        if ((! empty($vardef['studio']) && is_array($vardef['studio']) && ! empty($vardef['studio']['no_duplicate']) && $vardef['studio']['no_duplicate'] == true) || (strcmp($field_name, "name") == 0) || (isset($vardef['type']) && $vardef['type'] == 'name')) // bug #35767, do not allow cloning of name field
        {
