...
class EmailMan extends SugarBean
{
    ...
    var $verified_email_marketing_ids = [];
    ...
    function create_new_list_query(
        ...
        $query['select'] = "SELECT $this->table_name.* ,
					...
					(CASE related_type
						WHEN 'Contacts' THEN ".$this->db->concat('contacts', ['first_name', 'last_name'], '&nbsp;')."
						WHEN 'Leads' THEN ".$this->db->concat('leads', ['first_name', 'last_name'], '&nbsp;')."
						WHEN 'Accounts' THEN accounts.name
						WHEN 'Users' THEN ".$this->db->concat('users', ['first_name', 'last_name'], '&nbsp;')."
						WHEN 'Prospects' THEN ".$this->db->concat('prospects', ['first_name', 'last_name'], '&nbsp;')."
					END) recipient_name";
    ...
    function create_queue_items_query(
        $order_by, $where, $filter = [], $params = [], $show_deleted = 0, $join_type = '', $return_array = false,
        $parentbean = null, $singleSelect = false
    ) {
     ...
     * @param        string         from_address Email address of the sender, usually email address of the configured
     *                                           inbox.
     * @param        string         sender_id If of the user sending the campaign.
     * @param        array          macro_nv array of name value pair, one row for each replacable macro in email
     *                                       template text.
    ...
    function create_ref_email(
        $marketing_id, $subject, $body_text, $body_html, $campagin_name, $from_address, $sender_id, $notes, $macro_nv,
        $newmessage, $from_address_name
    ) {
        ...
        if (! empty($this->related_id) && ! empty($this->related_type)) {
            ...
            $this->ref_email->$rel_name->add($this->related_id, ['campaign_data' => $this->db->quote($campaignData)]);
        }
    ...
    function sendEmail($mail, $save_emails = 1, $testmode = false)
    {
        ...
        if ((! isset($module->email_opt_out) || ($module->email_opt_out !== 'on' && $module->email_opt_out !== 1 && $module->email_opt_out !== '1')) && (! isset($module->invalid_email) || ($module->invalid_email !== 'on' && $module->invalid_email !== 1 && $module->invalid_email !== '1'))) {
            ...
            $template_data = $this->current_emailtemplate->parse_email_template([
