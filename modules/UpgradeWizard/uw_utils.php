...
function commitMakeBackupFiles($rest_dir, $install_file, $unzip_dir, $zip_from_dir, $errors, $path = '')
{
	...
	if (file_exists($rest_dir) && is_dir($rest_dir)) {
		...
		$newFiles = findAllFiles(clean_path($unzip_dir.'/'.$zip_from_dir), []);
...
function removeFileFromPath($file, $path, $deleteNot = [])
{
	$removed = 0;
	$cur     = $path.'/'.$file;
	if (file_exists($cur)) {
...
function deleteAndOverWriteSelectedFiles($unzip_dir, $zip_from_dir, $delete_dirs)
{
	if ($delete_dirs != null) {
		foreach ($delete_dirs as $del_dir) {
			...
			$newFiles     = findAllFiles(clean_path($unzip_dir.'/'.$zip_from_dir.'/'.$del_dir), []);
...
function getAllCustomizedModules()
{
	...
	foreach ($modules as $mod) {
		...
		$modFiles = findAllFiles(clean_path(getcwd())."/modules/$mod", []);
...
function removeMd5MatchingFiles($deleteNot = [])
{
...
function getValidPatchName($returnFull = true)
{
	...
	$upgrade_content  = '';
	$upgrade_contents = findAllFiles($base_upgrade_dir, [], false, 'zip');
	...
	$upgradeToVersion = []; // fill with valid patches - we will only use the latest qualified found patch
...
function logThis($entry, $path = '')
{
	...
	if (! file_exists($log)) {
		...
		if (! is_resource($fp)) {
			Log::fatal('UpgradeWizard could not create the upgradeWizard.log file');
		...
		if (! is_resource($fp)) {
			Log::fatal('UpgradeWizard could not open/lock upgradeWizard.log file');
			die($mod_strings['ERR_UW_LOG_FILE_UNWRITABLE']);
		}
	}
	...
	if (@fwrite($fp, $line) === false) {
		Log::fatal('UpgradeWizard could not write to upgradeWizard.log: '.$entry);
		die($mod_strings['ERR_UW_LOG_FILE_UNWRITABLE']);
	}
 ...
 *
 * @author : nsingh
 * @desc   This function is to be used in the upgrade process to preserve changes/customaizations made to pre 5.1
 *         quickcreate layout. Prior to 5.1 we have been using editviewdefs as the base for quickcreatedefs. If a
 *         custom field was added to edit view layout, it was automatically picked up by the quick create. [Addresses
 *         Bug 21469] This function will check if customizations were made, and will create quickcreatedefs.php in the
 *         /cutom/working/$module_name directory.
 **/
function updateQuickCreateDefs()
{
	...
	foreach ($studio_modules as $modname) { //for each studio enabled module
		...
		if (! file_exists("modules/".$modname."/metadata/quickcreatedefs.php") && file_exists($editviewdefs) && ! file_exists($quickcreatedefs)) {
			//clone editviewdef and save it in custom/working/modules/metadata
			Log::debug("Copying editviewdefs.php as quickcreatedefs.php for the $modname module in custom/working/modules/$modname/metadata!");
			if (copy($editviewdefs, $quickcreatedefs)) {
				if (file_exists($quickcreatedefs) && is_readable($quickcreatedefs)) {
					...
					Log::debug("Failed to replace 'EditView' with QuickCreate because $quickcreatedefs is either not readable or does not exist.");
				}
			} else {
				Log::debug("Failed to copy $editviewdefs to $quickcreatedefs!");
			}
		}
	}
}
...
{
	$standardQueries = ['ALTER TABLE', 'DROP TABLE', 'CREATE TABLE', 'INSERT INTO', 'UPDATE', 'DELETE FROM'];
...
eoq;
	...
	$upgradeFiles     = findAllFiles(clean_path("$unzip_dir/$zip_from_dir"), []);
	$cache_html_files = [];
...
function getChecklist($steps, $step)
{
	...
	$skip = ['start', 'cancel', 'uninstall', 'end'];
	$j    = 0;
	$i    = 1;
...
function prepSystemForUpgrade()
{
	...
	if (! isset($subdirs) || empty($subdirs)) {
		$subdirs = ['full', 'langpack', 'module', 'patch', 'theme'];
	}
	...
	if ($upload_max_filesize_bytes < constant('SUGARCRM_MIN_UPLOAD_MAX_FILESIZE_BYTES')) {
		Log::debug("detected upload_max_filesize: $upload_max_filesize");
...
if (! function_exists('validate_manifest')) {
	...
	function validate_manifest($manifest)
	{
		...
		if (isset($manifest['acceptable_sugar_flavors']) && sizeof($manifest['acceptable_sugar_flavors']) > 0) {
			...
			if (! $flavor_ok) {
				return $mod_strings['ERROR_FLAVOR_INCOMPATIBLE']."<br />".$mod_strings['ERR_UW_FLAVOR'].$sugar_flavor."<br />".$mod_strings['ERR_UW_FLAVOR_2'].$manifest['acceptable_sugar_flavors'][0];
			}
		}
...
function unlinkUWTempFiles()
{
	...
	if (file_exists($tempDir) && is_dir($tempDir)) {
		$files = findAllFiles($tempDir, [], false);
		rsort($files);
		...
		// now do dirs
		$files = findAllFiles($tempDir, [], true);
 ...
 *               returned collection
 */
function uwFindAllFiles($dir, $theArray, $includeDirs = false, $skipDirs = [], $echo = false)
{
	// check skips
...
function testThis()
{
	$files = uwFindAllFiles(getcwd().'/test', []);
...
function testThis4($filePath, $fileNodes = [], $fileName = '')
{
...
function getFilesForPermsCheck()
{
	...
	$files    = uwFindAllFiles(".", [], true, $skipDirs, true);
...
function post_install_progress($progArray = '', $action = '')
{
	...
	if ($action == '' || $action == 'get') {
		...
		if (! is_array($upgrade_config[sizeof($upgrade_config)]['commit']['post_install'])) {
			$upgrade_config[sizeof($upgrade_config)]['commit']['post_install']                 = [];
...
function upgradeLocaleNameFormat($name_format)
{
	...
	if (! in_array($name_format, $sugar_config['name_formats'])) {
		$new_config                   = sugarArrayMerge($sugar_config['name_formats'], [$name_format => $name_format]);
	...
	foreach ($module_directories as $module_dir) {
		...
		if (file_exists($read_searchdefs_from) && file_exists($read_SearchFields_from)) {
			...
			if (! $found_sf1 && ! $found_sf2) {
				$searchdefs[$module_dir]['layout']['basic_search']['favorites_only']    = [
					'name'  => 'favorites_only',
					'label' => 'LBL_FAVORITES_FILTER',
					'type'  => 'bool',
				];
				$searchdefs[$module_dir]['layout']['advanced_search']['favorites_only'] = [
					'name'  => 'favorites_only',
					'label' => 'LBL_FAVORITES_FILTER',
					'type'  => 'bool',
				];
				$searchFields[$module_dir]['favorites_only']                            = [
					'query_type' => 'format',
					'operator'   => 'subquery',
 ...
 */
function upgradeModulesForTeamsets($filter = [])
{
			...
			if (file_exists($file.'.bak')) {
				...
				if ($php_mod_time - $bak_mod_time < 30) {
					...
					if ($fp) {
						...
						Log::error("Unable to update file contents in fix_dropdown_list for {$file}");
					} //if-else
				}
			}
			...
			if (isset($GLOBALS['app_list_strings']) && is_array($GLOBALS['app_list_strings'])) {
				...
				if ($touched) {
					...
					if ($fp) {
						...
						Log::error("Unable to update file contents in fix_dropdown_list.");
...
function upgradeDateTimeFields($path)
{
	//bug: 39757
	global $db;
	$meetingsSql = "UPDATE meetings SET date_end = ".$db->convert("date_start", 'add_time', [
			'duration_hours',
			'duration_minutes'
		]);
	$callsSql    = "UPDATE calls SET date_end = ".$db->convert("date_start", 'add_time', [
			'duration_hours',
			'duration_minutes'
		]);
 ...
 */
function merge_config_si_settings(
	$write_to_upgrade_log = false, $config_location = '', $config_si_location = '', $path = ''
) {
...
function upgrade_connectors()
{
	...
	if (! ConnectorUtils::updateMetaDataFiles()) {
		Log::fatal('Cannot update metadata files for connectors');
	}
 ...
 * remove_linkedin_config
 *
 * This function removes linkedin config from custom/modules/Connectors/metadata/display_config.php, linkedin connector
 * is removed in 6.5.16
 *
 */
	...
	if (file_exists($display_file)) {
		...
		if (! empty($modules_sources)) {
			...
			if (! write_array_to_file('modules_sources', $modules_sources, $display_file)) {
				//Log error
				Log::fatal("Cannot write \$modules_sources to ".$display_file);
			}
		}
	}
	...
	if (file_exists($search_file)) {
		...
		if (isset($searchdefs['ext_rest_linkedin'])) {
			...
			if (! write_array_to_file('searchdefs', $searchdefs, $search_file)) {
				//Log error
				Log::fatal("Cannot write \$searchdefs to ".$search_file);
			}
		}
	}
}
...
function removeSilentUpgradeVarsCache()
{
	...
	$silent_upgrade_vars_loaded = []; // Set to empty to reset it
...
function loadSilentUpgradeVars()
{
	...
	if (empty($silent_upgrade_vars_loaded)) {
		...
		if (! file_exists($cacheFile)) {
			...
			$silent_upgrade_vars_loaded = ['vars' => []];
		} else {
...
function getUWDirs()
{
	if (! class_exists('UploadStream')) {
		...
		return [$sugar_config['upload_dir']."/upgrades", $sugar_config['cache_dir']."upload/upgrades/temp"];
	} else {
		if (! in_array("upload", stream_get_wrappers())) {
			UploadStream::register(); // just in case file was copied, but not run
		}

		return ["upload://upgrades", sugar_cached("upgrades/temp")];
	}
}
 ...
 *
 * @param $globString String value used for glob search defaults to searching for all SearchFields.php files in modules
 *                    directory
 ...
 *
 * This is a helper function used in the upgrade process to fix upgrade_history entries so that the filename column
 * points to the new upload directory location introduced in 6.4 versions
 */
