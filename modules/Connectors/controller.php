...
class ConnectorsController extends SugarController
{
	...
	function action_SetSearch()
	{
		...
		$searchDefs   = isset($searchdefs) ? $searchdefs : [];
	...
	function action_RetrieveSourceDetails()
	{
		...
		$result = $source->fillBean(['id' => $record_id], $module);
		require_once('include/connectors/utils/ConnectorUtils.php');
		...
		$retArray['body']    = ! empty($str) ? str_replace(["\rn", "\r", "\n"], [
			'',
			'',
			'<br />'
		], $str) : $GLOBALS['mod_strings']['ERROR_NO_ADDITIONAL_DETAIL'];
	...
	function action_GetSearchForm()
	{
		...
		if (! empty($_REQUEST['source_id'])) {
			...
			$_searchDefs  = isset($searchdefs) ? $searchdefs : [];
			$_trueFields  = [];
		...
		if (! empty($_REQUEST['source_id'])) {
			...
			if (method_exists($source, $method)) {
				...
				echo $json->encode(['error'        => true,
									'errorMessage' => 'Could Not Find Function: '.$method.' in class: '.get_class($source)
				]);
			}
		} else {
			echo $json->encode(['error' => true, 'errorMessage' => 'Source Id is not specified.']);
		}
	}
	...
	function action_SaveModifyDisplay()
	{
		...
		foreach ($sources_modules as $source_id => $modules) {
			foreach ($modules as $module) {
				$searchdefs[$source_id][$module] = ! empty($connectorSearchDefs[$source_id][$module]) ? $connectorSearchDefs[$source_id][$module] : (! empty($originalSearchDefs[$source_id][$module]) ? $originalSearchDefs[$source_id][$module] : []);
			}
		}
	...
	{
		$mapping_sources = ! empty($_REQUEST['mapping_sources']) ? explode(',', $_REQUEST['mapping_sources']) : [];
		$mapping_values  = ! empty($_REQUEST['mapping_values']) ? explode(',', $_REQUEST['mapping_values']) : [];

		//Build the source->module->fields mapping
		$source_modules_fields = [];
		...
		foreach ($mapping_sources as $source_id) {
			if (empty($source_modules_fields[$source_id])) {
				...
				foreach ($mapping['beans'] as $module => $entry) {
					$source_modules_fields[$source_id][$module] = [];
				}
			}
		} //foreach
