...
class Email extends SugarBean
{
	...
	function sendEmailTest(
		$mailserver_url, $port, $ssltls, $smtp_auth_req, $smtp_username, $smtppassword, $fromaddress, $toaddress,
		$mail_sendtype = 'smtp', $fromname = ''
	) {
	...
	function email2Send($request)
	{
		...
		$emailAddressCollection = []; // used in linking to beans below
		// handle to/cc/bcc
		...
		foreach ($idsArray as $id) {
			...
			if (! empty($bean)) {
				...
				if (! empty($emailAddress)) {
					...
					if (in_array($module, ['Users', 'Employees'])) {
						$fullName = from_html($locale->getLocaleFormattedName($bean->first_name, $bean->last_name, '', $bean->title));
	...
	function _arrayToDelimitedString($arr)
	{
		...
		$separator = (isset($GLOBALS['sugar_config']['email_address_separator']) && ! empty($GLOBALS['sugar_config']['email_address_separator'])) ? $GLOBALS['sugar_config']['email_address_separator'] : ',';
	...
	function getStartPage($uri)
	{
		if (strpos($uri, '&')) { // "&" to ensure that we can explode the GET vars - else we're gonna trigger a Notice error
			...
			$start  = ['module' => '', 'action' => '', 'group' => '', 'record' => '', 'type' => ''];
	...
	function handleBody($mail)
	{
		...
		// HTML email
		if ((isset($_REQUEST['setEditor']) /* from Email EditView navigation */ && $_REQUEST['setEditor'] == 1 && trim($_REQUEST['description_html']) != '') || trim($this->description_html) != '' /* from email templates */ && $current_user->getPreference('email_editor_option', 'global') !== 'plain' //user preference is not set to plain text
		) {
	...
	function create_new_list_query(
		...
		$parentbean = null, $singleSelect = false
	) {
	...
	function fill_in_additional_list_fields()
	{
		...
		if ($row) {
			$this->attachment_image = SugarThemeRegistry::current()->getImage('attachment', '', null, null, '.gif', translate('LBL_ATTACHMENT', 'Emails'));
		} else {
